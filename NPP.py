...
class NPP:
    ...
    def update_table(self):
        ...
        run_button = tk.Button(self.input_frame, text="Run NPP", command=self.Calculate_NPP)
    ...
    def npp(self, job_list):
        ...
        while index < len(job_list) or job_queue:
            ...
            while index < len(job_list) and job_list[index][0] <= time:
                heapq.heappush(job_queue, (job_list[index][2], job_list[index][1], job_list[index][0], job_list[index][2]))
                index += 1

            if job_queue:
                # Pop the job with the shortest burst time from the heap
                priority, burst_time, arrival_time, job_name = heapq.heappop(job_queue)
                time += burst_time
                ct = time
                tat = ct - arrival_time
                wt = tat - burst_time
                gantt.append((job_name, time))
    ...
    def Calculate_NPP(self):
        ...
        gantt, completed = self.npp(process_list)
